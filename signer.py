...
#Get File extension for R
getExtension_R=lambda fileName: fileName.split('.')[-1]
...
#generate signature for R files
def generateSignature_R(symbol,name, date):

    signature_R = "{open}\n{open}**\n{open} * author :   \t{name}\n{open} * created : \t{date}\n{open}**\n{close}\n"

    return signature_R.format(open=symbol[0], close=symbol[1], name=name, date=date)
...
def signFile(filePath, symbol, author): 
    ...
    signature_R = generateSignature_R(symbol, author, date)
    f = open(filePath, 'r')
    ...
    if not fileAlreadySigned(''.join(contents), signature): 
        ...
        #R code block
    elif not fileAlreadySigned(''.join(contents), signature_R):
        contents.insert(0, signature_R)
        f.close()
        with open(filePath, 'w+') as f:
            f.write(''.join(contents))
        print(".R File successfully signed")
    else:
...
def Sign(dir_path, author, *args):
    ...
    if path.isfile(dir_path):
        ...
        symbol_R = getSymbol(getExtension_R(dir_path))
        ...
        if symbol:
            ...
            signFile(dir_path, symbol_R, author)
    ...
    #signing files in a folder
        ...
        for file in listdir(dir_path):
            ...
            symbol_R = getSymbol(getExtension(file))
            filePath = dir_path + "/" + file
            if symbol and path.isfile(filePath):
                signFile(filePath, symbol,author)

            elif symbol_R and path.isfile(filePath):
                signFile(filePath, symbol_R,author)
            else:
